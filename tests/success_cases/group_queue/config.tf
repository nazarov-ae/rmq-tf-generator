...
resource "rabbitmq_binding" "service3_queue_group_entity1" {
  source           = rabbitmq_exchange.entity1.name
  vhost            = "/"
  destination      = rabbitmq_queue.service3_queue_group.name
  destination_type = "queue"
  routing_key      = ""
}
resource "rabbitmq_binding" "service3_queue_group_entity2" {
